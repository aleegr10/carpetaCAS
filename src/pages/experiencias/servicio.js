...
const isVideo = (video, index) => {
    if(video === slideImagesCarrera[0]){
        return  <div key={index} style={{backgroundImage: `url(${video})`}} className="serSlide">
                </div>
    }else{
        return  <div key={index}className="serSlide">
                    <ReactPlayer 
                    url={video}
                    width='100%'
                    height='100%'
