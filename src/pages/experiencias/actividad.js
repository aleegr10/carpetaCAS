...
const isVideo = (video, index) => {
    if(video === slideImagesCarrera[0]){
        return  <div key={index} style={{backgroundImage: `url(${video})`}} className="actSlide">
                </div>
    }else{
        return  <div key={index}className="actSlide">
                    <ReactPlayer 
                    url={video}
                    width='100%'
                    height='100%'
            ...
            <div className='act'>
                ...
                <div className='actSlideContainer'>
                    {slideImagesVolun.map((slideImage, index)=> (
                        <div key={index} style={{backgroundImage: `url(${slideImage})`}} className="actSlide">
                        </div>
                    ))} 
                </div>
            </div>
